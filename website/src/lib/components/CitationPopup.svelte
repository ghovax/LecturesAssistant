<script lang="ts">
  ...
  let formattedPages = $derived.by(() => {
    ...
    if (
      ...
      sourcePages[sourcePages.length - 1] - sourcePages[0] ===
        sourcePages.length - 1
    ) {
...
>
  <div class="standard-header">
    ...
    <button
      class="btn btn-link btn-sm p-0 text-muted border-0 shadow-none"
      onclick={onClose}
    >
      <X size={16} />
    </button>
  ...
  {#if sourceFile}
    <div class="px-3 pb-3 source-info">
      <div
        class="text-muted"
        style="font-size: 0.75rem; line-height: 1.4; word-break: break-all;"
      >
        {sourceFile}
      </div>
      {#if formattedPages}
        <div
          ...
          style="font-size: 0.75rem; margin-top: 0.125rem;"
        >
          {formattedPages}
        </div>
      {/if}
